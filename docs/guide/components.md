...
Autobahn makes Hyli the ideal platform for high-throughput, low-latency applications, including payment networks, stablecoin settlement, and tokenized asset infrastructure.
...
## Practical example: a EUR stablecoin payment on Hyli

The following example walks through the lifecycle of a confidential EUR stablecoin transfer.

### Step 1: User initiates a payment

The user’s application submits a blob transaction to Hyli. The blob contains the encrypted payment details — recipient, amount, and a commitment to the private inputs.

At this point, nothing has been settled.

### Step 2: Application processes the transaction

The stablecoin issuer’s application monitors the Hyli node. It consumes the new blob, applies its confidential transfer logic offchain, and updates its internal state.

The execution logic can be written in any language that supports a compatible proving scheme — Rust with RISC-0 or SP1, or Noir for circuit-based logic. The private transaction details remain entirely offchain.

### Step 3: User receives confirmation

The user receives immediate confirmation from the application once execution is complete. The underlying proof has not yet been settled, but since the application generates validity proofs, it cannot misrepresent the outcome.

This delivers sub-second confirmation for payment operations without waiting for proof generation.

### Step 4: Proof is generated and settled

The application asynchronously generates a cryptographic proof of the valid state transition and submits a proof transaction to Hyli. The proof confirms that the transfer was executed correctly — including balance validity and reserve backing — without exposing private payment details.

Hyli verifies the proof and settles the new state onchain. What was sequenced is now confirmed: the issuer updates its state and the user receives final settlement confirmation.
